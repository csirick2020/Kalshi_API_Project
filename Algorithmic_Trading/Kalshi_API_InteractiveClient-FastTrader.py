# Kalshi_API_Interactive_Client-FastTrader
# FastTrader allows you to trade more contracts- Buy and sell 'sleep' times are cut in half
...
SEVEN_AND_A_HALF_MINUTES = SECONDS_PER_MINUTE * 7.5  # 'Buy' sleep time
TWO_MINUTES = SECONDS_PER_MINUTE * 2  # 'Sell' sleep time
...
while True:
    if break_outer is False:
                    ...
                    if break_mid is False:
                        try:
                            ...
                            if 1 > event_type > 4:  # User did not type a number between 1-4
                                ...
                                while True:
                                    event_date = input(
                                        "Please enter the year/month/day (of the daily Up/Down market you want to work with) in this exact format...\nYY/MON/DD\nExample: 23/DEC/10\n*If you do not enter the year/month/day in the exact format mentioned, the program will break and you will have to start over.\n*If you choose a date on which the market is not open for trading, the program will break and you will have to start over.\n(Daily Up/Down market also skips days sometimes, which will cause the program to break if you enter a day on which this market is not open for trading.\nYou enter here: "
                                    ).upper()
                                    # Remove "/" characters from the user input
                                    event_date = event_date.replace("/", "")
                                    break_mid = True
                                ...
                                print()
                                print("---Weekly Bracket---")
                                print()
                                while True:
                                    event_date = input(
                                        "Please enter the year/month/day (of the weekly bracket you want to work with) in this exact format...\nYY/MON/DD\nExample: 23/DEC/10\n*If you do not enter the year/month/day in the exact format mentioned, the program will break and you will have to start over.\n*If you choose a date on which the market is not open for trading, the program will break and you will have to start over.\n(The weekly bracket date must also fall on a Friday (since this market will only ever be active on Fridays). Entering a date that is not a Friday will cause the program to break and you will have to start over.\nYou enter here: "
                                    ).upper()
                                    # Remove "/" characters from the user input
                                    event_date = event_date.replace("/", "")
                                    break_mid = True
                                ...
                                while True:
                                    event_date = input(
                                        "Please enter the year/month/day (of the end-of-year bracket you want to work with) in this exact format...\nYY/MON/DD\nExample: 23/DEC/10\n*If you do not enter the year/month/day in the exact format mentioned, the program will break and you will have to start over.\n*If you enter any date other than the valid last trading day of the year, the program will break and you will have to start over.\nYou enter here: "
                                    ).upper()
                                    # Remove "/" characters from the user input
                                    event_date = event_date.replace("/", "")
                                    break_mid = True
        ...
        # Deal with final conditionals (to close the loop-dee-loo)!
        except ValueError:  # User did not type an integer
            print()
            print("Invalid entry. Please type in a valid integer.")
            print()
...
print()
ind_mrkt_tick = input(
    "Which market would you like to work with?\nPlease copy and paste one of the tickers from above. ***Example (format): NASDAQ100Y-23DEC29-T14999.99\n(*In the case of the 'Daily Up/Down' market, there is only one ticker.)\nPlease note that if the ticker is copied incorrectly, the program will not be able to read it and you will have to start over.\n(Also, be aware that there are certain exceptions with holidays, etc.- Meaning that if a ticker doesn't show up (and you've made it to this point without errors),\nit is one of those exceptions and you'll want to restart the program and choose a different event option (like 'Daily' instead of 'Weekly', for example)\nPaste market ticker here:  "
)
        ...
        if choose_take_profit == '1':
            ...
            if avg_yes <= sell_price:
                ...
                if contracts_sold < contracts_bought:
                    time.sleep(TWO_MINUTES)  # Sell time sleep quicker than buy sleep
                    continue
                else:
...
print()
start_time = input("Please enter the start time for your 'time of day' range in the following 24 hr. format.\n'HH:MM' --- Example--- 08:30 = 8:30 AM /// 12:07 = 12:07 PM /// 16:47 = 4:47 PM\nYou enter here: ")
end_time = input("Please enter the end time for your 'time of day' range in the same (24 hr.) format.\nYou enter here: ")
print()
        ...
        if min_price_range <= avg_yes <= max_price_range:
            ...
            if start_time <= current_time <= end_time:
                ...
                if contracts_bought < contract_ceiling:
                    time.sleep(SEVEN_AND_A_HALF_MINUTES)  # sleep for longer after a "buy" to make sure price is staying consistent
                    continue
                else:
